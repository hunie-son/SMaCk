...
// A pointer representing an instacne of L1 insctuction cache pnp tool
struct l1ipp{
	//block of exeecutable memory for setting
  void *memory;
  //array whcih cache sets are being moinitored
  uint8_t monitored[L1I_SETS];
  //pointers to start each cache line within the memory block
  void *sets[L1I_SETS];
  // num of cache sets monitored
  int nsets;
};
...
//allocate executable memory for probe
 ...
 //jumps in memory to create chain of calls
//allocates memory and set up a serioes of function that can be called to access specific cache sets in the l1i cache
//
...
//specific cache sets to the list of monitored sets
//
...
//Removes a specific cache set from the list
...
//stop monitoring all cache sets
